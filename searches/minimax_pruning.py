...
def minimax_tree(node, is_maximizing,alpha=float('-inf'),beta=float('inf')):
    ...
    if is_maximizing:
        ...
        for child in node.children:
            value = minimax_tree(child, False, alpha, beta)
            best_val = max(best_val, value)
            alpha = max(alpha, best_val)
            if alpha >= beta:
                print("Pruning at node: ", child.name)
                return best_val
        ...
        best_val = float('inf')
        for child in node.children:
            value = minimax_tree(child, True, alpha, beta)
...
# Calculation
result = minimax_tree(root, True, float('-inf'), float('inf'))
